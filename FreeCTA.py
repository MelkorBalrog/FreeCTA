...
of an autonomous system’s subsystems. It produces an Assurance Level (on a scale 
...
### 1. Deriving an Assurance Level from Base Inputs
When only Confidence and Robustness values are provided, the tool “inverts” these 
inputs to yield a base Assurance Level. In this method, low confidence and low robustness 
...
– Mixed values yield intermediate assurance levels.

---

### 2. Aggregating Assurance Levels from Child Nodes

When a parent node aggregates assurance levels from its children, the aggregation method 
...
### 3. Decomposing a Parent Assurance Level into Child Targets

A parent node’s overall assurance requirement can be decomposed into target assurance levels 
...
These rules ensure that when children’s assurance levels are aggregated (using the AND or OR rules), 
...
- **General Rule (for most nodes):**  
  **Final Assurance Level = (Aggregated Child Assurance + Highest Parent Severity) ÷ 2**  
...
- **For Vehicle Level Functions:**  
  ...
  **Adjusted Assurance = (2 × Computed Assurance) – (Node’s Own Severity)**  
  This modification increases the assurance level when the potential impact is high.
...
- **Adjustment with Severity:**  
  The final assurance level is adjusted by incorporating the severity (using the highest parent severity unless the node is a Vehicle Level Function, in which case its own severity is used).

- **Decomposition and Aggregation:**  
  The parent node’s assurance requirement can be decomposed into target assurance levels for its children (see Decomposition Guidelines above), and child assurance levels are aggregated (using the AND/OR rules) to reconstruct the parent’s overall requirement.
...
VALID_SUBTYPES = {
    ...
    "Assurance Level": ["Vehicle Level Function"]
}
...
class ADRiskAssessmentHelper:
    ...
    def aggregate_assurance_and(self,child_levels):
        """
        Combine a list of children’s assurance levels for an AND gate,
    ...
    def derive_assurance_from_base(self,conf_values, rob_values):
        ...
        compute a single 'inverted' assurance level from 1..5,
    ...
    def calculate_assurance_recursive(self, node, all_top_events, visited=None):
        ...
        if node.node_type.upper() == "TOP EVENT":
            ...
            node.display_label = f"Assurance Level [{level_map[final]}]"
            ...
            node.display_label = f"Assurance Level [{level_map[combined]}]"
            node.detailed_equation = (
                f"Base Assurance from children = {base_assurance if base_assurance is not None else 'N/A'}\n"
                f"Composite Assurance from gates = {composite_assurance if composite_assurance is not None else 'N/A'}\n"
...
class FaultTreeApp:
    def __init__(self, root):
        ...
        process_menu.add_command(label="Calc Assurance Level", command=self.calculate_overall, accelerator="Ctrl+R")
        ...
        view_menu.add_command(label="FMEA Table", command=self.show_fmea_table)
    ...
    def generate_recommendations_for_top_event(self, node):
        # Determine the assurance level based on the node’s quantitative score.
        ...
        rec_text = f"<b>Recommendations for Assurance Level {level}:</b><br/>"
    ...
    def build_argumentation(self, node):
        ...
        if node.node_type.upper() == "TOP EVENT":
            ...
            header += (
                f"Assurance Level Explanation:<br/>"
                f"Based on the aggregated scores of its child nodes, this top event has been assigned an Assurance Level of <b>{assurance_descr}</b> "
        ...
        for uid in sorted(unique_ids):
            ...
            subtype = n.input_subtype if n.input_subtype is not None else (
                ...
                else VALID_SUBTYPES.get("Assurance Level", ["Default"])[0]
            )
    ...
    def auto_create_argumentation(self, node, suppress_top_event_recommendations=False):
        ...
        if node.node_type.upper() == "TOP EVENT" and not suppress_top_event_recommendations:
            ...
            severity_str = f"{node.severity}" if node.severity is not None else "N/A"
            header = (
                f"Assurance Level Explanation:\n"
                f"This top event is assigned an Assurance Level of '{assurance_descr}' with a severity rating of {severity_str}.\n\n"
            )
    ...
    def generate_argumentation_report(self, event):
        ...
        followed by the assurance level explanation (including the rationale behind its severity)
        ...
        text = (
            ...
            f"Assurance Level Explanation:<br/>"
            f"This top event is assigned an Assurance Level of <b>{assurance_level}</b> "
    ...
    def get_extra_recommendations_list(self, description, level):
        """
        Given a node's description and its assurance level, return a list of extra recommendations.
    ...
    def get_extra_recommendations_from_level(self,description, level):
        """
        Given a node's description and its assurance level (1-5), look up keywords from the level's 
    ...
    def get_recommendation_from_description(self, description, level):
        """
        Given a node's description and its assurance level, this function iterates over all keys 
    ...
    def build_dynamic_recommendations_table(events, app):
        ...
        [Event Name, Assurance Level, Severity, Description, Rationale, Dynamic Recommendations].
    ...
    def generate_top_event_summary(self, top_event):
        ...
        that includes:
          ...
          - The required assurance level (with numeric score) and the severity rating.
        ...
        summary_sentence = (
            ...
            f"  - Required Assurance Level: {assurance_descr} (Score: {overall_assurance:.2f})\n"
    ...
    def get_node_fill_color(self, node):
        ...
        if "Assurance Level" in label:
            base_type = "Assurance Level"
        ...
        color_mapping = {
            ...
            "Assurance Level": {"Vehicle Level Function": "pink", "Functionality": "lightyellow","Capability": "turquoise", "Safety Mechanism": "yellow"},
